...
from libs.robosuite.models.mjcf_utils import string_to_array
...
def angular_error_euler(target_orientation, achieved_orientation):
    return target_orientation - achieved_orientation


def goal_distance(goal_a, goal_b):
    # TODO add orientation as in https://math.stackexchange.com/questions/90081/quaternion-distance
    assert goal_a.shape == goal_b.shape
    return np.linalg.norm(goal_a - goal_b, axis=-1)


def quat_to_euler(quat, degrees=True):
    # input Mujoco: quat=[w,x,y,z] => change it to [x,y,z,w]
    quat = quat[[1, 2, 3, 0]]
    rot = Rot.from_quat(quat, True)
    return rot.as_euler('xyz', degrees=degrees)


def euler_to_quat(euler):
    # input Mujoco: quat=[w,x,y,z] => change it to [x,y,z,w]
    rot = Rot.from_euler("xyz", euler, True)
    quat = rot.as_quat()
...
def set_mesh_scale(mesh_asset, current_size, desired_size, default_scale=None):
    if desired_size is None:
        return
    else:
        from libs.robosuite.models.mjcf_utils import array_to_string
        if default_scale is None:
            default_scale = string_to_array(mesh_asset.get("scale", array_to_string(np.ones(3))))
        # new_scale_x = self.length * default_scale / default_length
        # new_scale_y = self.width * default_scale / default_width
        # new_scale_z = self.height * default_scale / default_height
        new_scale = desired_size * default_scale / current_size
        mesh_asset.set("scale", array_to_string(new_scale))
        return new_scale
