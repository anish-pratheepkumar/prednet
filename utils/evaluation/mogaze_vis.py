...
from models.prednet.data_utils import normalize_data as prednet_normalize_data, \
    post_process_mogaze as prednet_post_process_mogaze
from models.prednet.train_prednet import check_action
from models.red.RED_data_utils import normalize_data as red_normalize_data, \
    post_process_mogaze as red_post_process_mogaze
from utils.data_mogaze_vis.mogaze_utils import build_env


def apply_mogaze_settings(avoid_goal, test=True):
    if test:
...
def predict_test_mogaze():
    ...
    with sess.as_default():
        ...
        #                                                 Start prediction
        # --------------------------------------------------------------------------------------------------------------
        for traj in all_trajs[::5]:  # skip 5 frames to align with trained model
            ...
            if len(normed_prev_data) == config.SEQ_LENGTH_IN:
                ...
                #                                dtype=float)
                # else:
                encoder_inputs = np.zeros((1, config.SEQ_LENGTH_IN - 1, mogaze_size), dtype=float)
                decoder_inputs = np.zeros((1, config.SEQ_LENGTH_OUT, mogaze_size), dtype=float)
                decoder_outputs = np.zeros((1, config.SEQ_LENGTH_OUT, mogaze_size), dtype=float)

                encoder_inputs = normed_prev_data[0:config.SEQ_LENGTH_IN - 1, :decoder_inputs.shape[2]]
...
if __name__ == '__main__':
    ...
    config.ARCHITECTURE = "prednet"
    apply_mogaze_settings(avoid_goal=False)
    predict_test_mogaze()
